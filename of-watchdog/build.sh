...
if [ ! "$http_proxy" = "" ]
then
    docker build --no-cache --build-arg "https_proxy=$https_proxy" --build-arg "http_proxy=$http_proxy" -t jinyuzhu/of-watchdog:build .
else
    docker build -t jinyuzhu/of-watchdog:build .
fi
...
docker build --no-cache --build-arg PLATFORM="" -t jinyuzhu/of-watchdog:latest-dev-x86_64 . -f Dockerfile.packager

docker create --name buildoutput jinyuzhu/of-watchdog:build echo
