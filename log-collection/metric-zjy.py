...
def replace_node_label(node_label):
    ...
    node_label = re.sub(r':\d{4}', ':RD_PORT', node_label)
    node_label = re.sub(r':\d{3}', ':RD_PORT', node_label)
    node_label = re.sub(r':\d{2}', ':RD_PORT', node_label)
    node_label = re.sub(r'[\d\.]+:', 'RD_IP:', node_label)
    node_label = re.sub(r'_.{10}-.{5}', '_UUID', node_label)
    return node_label
...
def calculate_all(gt_graph, sample_graph):
    ...
    gt_edges = list(set(gt_edges))
    gt_nodes = list(set(gt_nodes))
    sample_edges = list(set(sample_edges))
    sample_nodes = list(set(sample_nodes))

    print(f"attack nodes: {len(gt_nodes)}, edges: {len(gt_edges)}")
    print(f"sample nodes: {len(sample_nodes)}, edges: {len(sample_edges)}")

    print("in attack but not in sample: ")
    for attack_edge in gt_edges:
        if attack_edge not in sample_edges:
            print(f"--- {attack_edge}")
...
def calc(gt, sample, info):
    ...
    TP = sum(min(gt_count[node], sample_count[node]) for node in gt_count)
    FP = sum(sample_count.values()) - TP
    FN = sum(gt_count.values()) - TP
...
def metric_benign(gt, sample, name):
    gt_count = Counter(gt)
    sample_count = Counter(sample)
    remaining_count = sample_count - gt_count
    remaining_total = sum(remaining_count.values())
    print(f"BENIGN {name}: {remaining_total}")

def metric_main_alls(gt_file, sample_path):
    gt_graph = load_graph(gt_file)
    sample_graph = load_graph(sample_path)
    gt_nodes = list(map(lambda x: replace_node_label(x), gt_graph.nodes()))
    sample_nodes = list(map(lambda x: replace_node_label(x), sample_graph.nodes()))
    gt_edges = list(map(lambda x: replace_node_label(x[0]) + " => " + replace_node_label(x[1]), gt_graph.edges()))
    sample_edges = list(map(lambda x: replace_node_label(x[0]) + " => " + replace_node_label(x[1]), sample_graph.edges()))
    metric_benign(gt_nodes, sample_nodes, "nodes")
    metric_benign(gt_edges, sample_edges, "edges")
...
if __name__ == "__main__":
    ...
    for attack in attacks:
        gt_base = find_latest_timestamped_directory("/home/ubuntu/detection24/log-collection/output-zch-1105", attack_type=attack, file_type='gt')
        rc1_base = find_latest_timestamped_directory("/home/ubuntu/detection24/log-collection/output-zch-1105", attack_type=attack, file_type='rc1')
        rc2_base = find_latest_timestamped_directory("/home/ubuntu/detection24/log-collection/output-zch-1105", attack_type=attack, file_type='rc2')
        rc3_base = find_latest_timestamped_directory("/home/ubuntu/detection24/log-collection/output-zch-1105", attack_type=attack, file_type='rc3')
        print(gt_base)
        ...
        metric_main_alls(gt, rc1_all)
        
        metric_main("rc2", gt, rc2)
        metric_main_alls(gt, rc2_all)
        
        metric_main("rc3", gt, rc3)
        metric_main_alls(gt, rc3_all)
