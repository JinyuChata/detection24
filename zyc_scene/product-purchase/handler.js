...
const constants = {
	...
	URL_PLACE_ORDER: process.env.URL_PLACE_ORDER
};
...
const api = {
	purchaseProduct: async (event, context, callback) => {
		...
		if (event.body.test && event.body.test == "place-order"){
			const res = await request(functions.getRequestObject(event.body, constants.URL_PLACE_ORDER), constants.URL_PLACE_ORDER);
			callback(null, res);
			return;
		}
